    ...
    <div class="content-wrapper">
        <div class="border border-blue-100 rounded-lg p-2">
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-bold text-blue-600">THÙNG RÁC BANNER</h2>
                    ...
                    @foreach ($list as $item)
                        <tr>
                            <td class="border border-gray-300 p-2">
                                <img src="{{ asset('storage/' . $item->image) }}" class="w-full h-20 object-cover" alt="{{ $item->name }}">
                            </td>
                            <td class="border border-gray-300 p-2">{{ $item->name }}</td>
                            <td class="border border-gray-300 p-2">{{ $item->position }}</td>
                            <td class="border border-gray-300 p-2 text-center">
                                <a href="{{ route('banner.restore', ['banner' => $item->id]) }}" class="fa-solid fa-rotate-left text-blue-500 text-2xl"></a>

                                <form action="{{ route('banner.destroy', ['banner' => $item->id]) }}" method="post" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button>
