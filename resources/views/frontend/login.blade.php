    ...
    <div class="w-[800px] h-[500px] bg-white shadow-lg rounded-xl flex overflow-hidden">
        ...
        <div class="w-1/2 bg-white flex flex-col justify-center items-center p-8 relative z-10">
            ...
            <form action="{{ route('login.store') }}" method="POST" class="w-full">
                @csrf
                <input type="text" name="username" placeholder="Tên đăng nhập / Email" class="border border-gray-300 rounded-lg px-4 py-2 w-full mb-4 focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                <input type="password" name="password" placeholder="Mật khẩu" class="border border-gray-300 rounded-lg px-4 py-2 w-full mb-4 focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 w-full focus:outline-none focus:ring-2 focus:ring-blue-500">Đăng nhập</button>
            </form>

            <div class="mt-4 text-sm text-gray-600">
                Chưa có tài khoản? 
                <a href="{{ route('user.register') }}" class="text-blue-500 hover:underline">Đăng ký</a>
            </div>
        </div>

        <!-- Right Side (Image / Optional Text) -->
